...
attachment_dir = 'C:/Users/ELCOT/Desktop/'
...
def signin_view(request):
    ...
    if request.method == 'POST':
        text1 = "Welcome to the voice based email signup page.If you are an existing user say login or else say continue to fill this page."
        texttospeech(text1, file + i)
        i = i + str(1)
        answer=speechtotext(10)
        print(answer)
        if(answer=="login"):
                texttospeech("You will now be redirected to the login page.", file + i)
                i = i + str(1)
                ...
                flag=True
                while(flag):
                    texttospeech("Say your name",file+i)
                    name=speechtotext(7)
                    i=i+str(1)
                    if name != 'N':
                        texttospeech("You meant " + name + " say okay to confirm or no to Say again", file + i)
                        i = i + str(1)
                        say = speechtotext(3)
                       
                        print(say)
                        if say == 'ok' or say == 'Okay':
                                flag = False
                    else:
                ...
                name = name.replace(' ', '')
                name = name.lower()
                name = convert_special_char(name)
                print(name)
                flag = True
                while (flag):
                    texttospeech("Say your Email", file + i)
                    addr=speechtotext(10)
                    i = i + str(1)  
                    if addr != 'N':
                            texttospeech("You meant " + addr + " say okay to confirm or no to Say again", file + i)
                            i = i + str(1)
                            say = speechtotext(3)
                            print(say)
                            if say == 'ok' or say == 'Okay':
                ...
                flag = True
                while (flag):
                    texttospeech("Say your password", file + i)
                    i = i + str(1)
                    passwrd = speechtotext(15)
                    ...
                    if validate_password(passwrd):  
                        ...
                        if passwrd != 'N':
                            texttospeech("You meant " + passwrd + " say okay to confirm or no to Say again", file + i)
                            i = i + str(1)
                            say = speechtotext(3)
                            if say == 'ok' or say == 'Okay':
                                flag = False
                        ...
                        texttospeech("password length should be more than 8 characters:",file + i)
                        i= i + str(1)
                ...
                print(passwrd)
                texttospeech("Congratulations. You have signed in  successfully. You will now be redirected to the login page.", file + i)
                i = i + str(1)
                return JsonResponse({'result' : 'success'})
    detail  = UserDetails()
    detail.email = addr
    ...
    return render(request, 'homepage/signin.html', {'detail' : detail}) 
...
def options_view(request):
    ...
    if request.method == 'POST':
        ...
        i = i + str(1)
        while(flag):
            texttospeech("  To open compose folder say compose.To open Inbox folder say Inbox.To open Sent folder say Sent.To open Trash folder say Trash.  To log out say logout.  Do you want me to repeat?", file + i)
            i = i + str(1)
            say = speechtotext(3)
            if say == 'No' or say == 'no':
                flag = False
        texttospeech("Say your desired action", file + i)
        i = i + str(1)
        act = speechtotext(5)
        act = act.lower()
        if act == 'compose':
            ...
            texttospeech("Invalid action. Please try again. after some time", file + i)
            i = i + str(1)
            return JsonResponse({'result': 'failure'})
...
def compose_view(request):
    ...
    if request.method == 'POST':
        ...
        while flag1:
            while flag: 
                texttospeech("Say receiver's email address:", file + i)
                i = i + str(1) 
                to = speechtotext(15)
                if to != 'N':
                    texttospeech("You meant " + to + " say okay to confirm or no to Say again", file + i)
                    i = i + str(1)
                    say = speechtotext(5)
                    if say == 'ok' or say == 'Okay' or say=='okay':
                        toaddr.append(to)
                        flag = False
                else:
                    texttospeech("could not understand what you meant", file + i)
                    i = i + str(1)
            texttospeech("Do you want to Say more recipients ?  Say okay or no.", file + i)
            i = i + str(1)
            say1 = speechtotext(3)
        ...
        flag = True
        while (flag):
            texttospeech("Say subject", file + i)
            i = i + str(1)
        ...
        flag = True
        while flag:
            texttospeech("Say body of the mail", file + i)
            i = i + str(1)
            body = speechtotext(10)
        ...
        texttospeech("any attachment? say okay or no", file + i)
        i = i + str(1)
        x = speechtotext(3)
        x = x.lower()
        if x == 'okay' or x == 'ok':
            texttospeech("Do you want to record an audio and send as an attachment? say okay or no", file + i)
            i = i + str(1)
            say = speechtotext(2)
            say = say.lower()
            if say == 'ok' or say == 'okay':
                texttospeech("Say filename.", file + i)
                i = i + str(1)
                ...
                print(filename)
                texttospeech("Say your audio message.", file + i)
                i = i + str(1)
                ...
                texttospeech("Say filename with extension", file + i)
                i = i + str(1)
                filename = speechtotext(5)
...
def reply_mail(msg_id, message):
    ...
    while(flag):
        ...
        try:
            ...
            texttospeech("Your reply could not be sent. Do you want to try again? Say okay or no.", file + i)
            i = i + str(1)
            act = speechtotext(3)
            act = act.lower()
            if act != 'okay' or act!= 'ok':
                flag = False
...
def read_mails(mail_list,folder):
    ...
    while flag :
        ...
        flag1 = True
        while flag1:
            texttospeech("Say the email number of mail you want to read.",file + i)
            i = i + str(1)
            n =speechtotext(10)
        
            m=w2n.word_to_num(n)
            print(m)
            texttospeech("You meant " + str(n) + ". Say okay or no.", file + i)
            i = i + str(1)
        
            say = speechtotext(2)
            say = say.lower()
            if say == 'okay' or say == 'ok':
                flag1 = False 
      
        msgid = to_read_list[m - 1]
        ...
        if Body != '':
            texttospeech("The body of the mail is "+Body, file + i)
            i = i + str(1)
        else:
        ...
        if folder == 'inbox':
            texttospeech("Do you want to reply to this mail? Say okay or no. ", file + i)
            i = i + str(1)
            ans = speechtotext(3)
            ans = ans.lower()
            print(ans)
            if ans == "okay" or ans == 'ok':
                reply_mail(Msg_id, message)
 
        if folder == 'inbox' or folder == 'sent':
            texttospeech("Do you want to delete this mail? Say okay or no. ", file + i)
            i = i + str(1)
            ans = speechtotext(3)
            ans = ans.lower()
            print(ans)
...
def inbox_view(request):
    ...
    if request.method == 'POST':
        ...
        text = "You have reached your inbox. There are " + str(len(mail_list)) + " total mails in your inbox. You have " + str(no) + " unread emails" + ". To read unread emails say unread. To search a specific email say search. To go back to the menu page say back. To log out say log out."
        ...
        while(flag):
            ...
            if act == 'unread':
                ...
                emailid = ""
                while True:
                    texttospeech("Say email ID of the person who's email you want to search.", file + i)
                    i = i + str(1)
                    emailid =speechtotext(10)
                    texttospeech("You meant " + emailid + " say okay to confirm or no to Say again", file + i)
                    i = i + str(1)
                    yn = speechtotext(5)
                    yn = yn.lower()
            ...
            texttospeech("If you wish to do anything else in the inbox or log out of your mail say okay or else say no.", file + i)
            i = i + str(1)
            ans = speechtotext(3)
            ans = ans.lower()
            if ans == 'okay' or ans == 'ok':
                flag = True
                texttospeech("Say your desired action. Say unread, search, back or log out. ", file + i)
                i = i + str(1)
        texttospeech("You will now be redirected to the menu page.", file + i)
        i = i + str(1)
        conn.logout()
...
def sent_view(request):
    ...
    if request.method == 'POST':
        ...
        result1, data1 = conn.search(None, "ALL")
        mail_list = data1[0].split()
        text = "You have reached your sent mails folder. You have " + str(len(mail_list)) + " mails in your sent mails folder. To search a specific email say search. To go back to the menu page say back. To log out say log out."
        texttospeech(text, file + i)
        i = i + str(1)
        flag = True
        while (flag):
            ...
            if act == 'search':
                ...
                emailid = ""
                while True:
                    texttospeech("Say email ID of receiver.", file + i)
                    i = i + str(1)
                    emailid = speechtotext(10)
                    texttospeech("You meant " + emailid + " say okay to confirm or no to Say again", file + i)
                    i = i + str(1)
            ...
            texttospeech("If you wish to do anything else in the sent mails folder say okay or else say no.", file + i)
            i = i + str(1)
            ans = speechtotext(3)
            ans = ans.lower()
            if ans == 'okay' or ans =='ok':
                flag = True
                texttospeech("Say your desired action. Say search, back or log out. ", file + i)
                i = i + str(1)
        texttospeech("You will now be redirected to the menu page.", file + i)
        i = i + str(1)
        conn.logout()
...
def trash_view(request):
    ...
    if request.method == 'POST':
        ...
        result1, data1 = conn.search(None, "ALL")
        mail_list = data1[0].split()
        text = "You have reached your trash folder. You have " + str(len(mail_list)) + " mails in your trash folder. To search a specific email say search. To go back to the menu page say back. To log out say log out."
        texttospeech(text, file + i)
        i = i + str(1)
        flag = True
        while (flag):
            ...
            if act == 'search':
                ...
                emailid = ""
                while True:
                    texttospeech("Say email ID of sender.", file + i)
                    i = i + str(1)
                ...
                passwrd = ""
                texttospeech(
                    "You have been logged out of your account and now will be redirected back to the signin page.",
                    file + i)
                i = i + str(1)
                return JsonResponse({'result': 'logout'})
            ...
            texttospeech("If you wish to do anything else in the trash folder or log out of your mail say okay or else say no.", file + i)
            i = i + str(1)
            ans = speechtotext(3)
            ans = ans.lower()
            print(ans)
